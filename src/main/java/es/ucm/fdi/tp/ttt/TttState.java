...
public class TttState extends GameState<TttState, TttAction> {
    ...
    public TttState(TttState prev, int[][] board, boolean finished, int winner) {
    	super(2); //gameState, playerCount (numero de jugadores)
    	this.dim = prev.dim;
        this.board = board;
    ...
    private static boolean isWinner(int[][] board, int playerNumber, 
        ...
        boolean won = true;
        for (int i=0, x=x0, y=y0; won && i<board.length; i++, x+=dx, y+=dy) { //son dos for en 1. board[y][x], x+=dx (x++)...
           //comprueba si son 3 las casillas que tiene cogidas.
    ...
    public static boolean isWinner(int[][] board, int playerNumber) {
        ...
        for (int i=0; !won && i<board.length; i++) {
            if (isWinner(board, playerNumber, 0, i, 1, 0)) won = true; //comprueba cada pos de la fila (0,0,1,0)
